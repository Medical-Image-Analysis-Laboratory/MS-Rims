...
def get_model_graph(patch_size, *, n_channels = 1, weight_ce=[1., 1.]):
    ...
    # Input Layer
    # Layer #1
    conv1 = conv_3d(x, 32, 3,
                    activation='tanh', weights_init='xavier',
                    padding='same', regularizer="L2")

    conv1 = conv_3d(conv1, 32, 3,
                    activation='tanh', weights_init='xavier',
                    padding='same', regularizer="L2")
    ...
    # Layer #2
    conv2 = conv_3d(pool1, 64, 3,
                    activation='tanh', weights_init='xavier',
                    padding='same', regularizer="L2")

    conv2 = conv_3d(conv2, 64, 3,
                    activation='tanh', weights_init='xavier',
                    padding='same', regularizer="L2")
    ...
    # Layer #3
    conv3 = conv_3d(pool2, 128, 3,
                    activation='tanh', weights_init='xavier',
                    padding='same', regularizer="L2")

    conv3 = conv_3d(conv3, 128, 3,
                    activation='tanh', weights_init='xavier',
                    padding='same', regularizer="L2")
    ...
    # Dense layer
    pred_conv = conv_3d(pool3, 1, 1,
                        activation='linear', padding='valid')
    fully1 = fully_connected(pred_conv, 128,
                             activation='linear', weights_init='truncated_normal')
    fully2 = fully_connected(fully1, 64,
                             activation='linear', weights_init='truncated_normal')

    pred = fully_connected(fully2, 2,
                           activation='linear')
