...
def get_model_graph(patch_size, *, n_channels = 1, weight_ce=[1., 1.]):
    ...
    # PHASE layers
    # Layer #1
    conv1 = conv_3d(main, 32, 3, activation='tanh', weights_init='xavier', padding='same', regularizer="L2")
    conv1 = conv_3d(conv1, 32, 3, activation='tanh', weights_init='xavier', padding='same', regularizer="L2")
    pool1 = max_pool_3d(conv1, 2)
    main_concat = tf.concat([att_pool1, pool1], 4)
    # Layer #2
    conv2 = conv_3d(main_concat, 64, 3, activation='tanh', weights_init='xavier', padding='same', regularizer="L2")
    ...
    fully1 = fully_connected(pred_conv, 256,
