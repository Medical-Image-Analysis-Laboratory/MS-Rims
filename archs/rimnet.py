...
def get_model_graph(patch_size, *, n_channels = 1, weight_ce=[1., 1.]):
    ...
    phase, t2, flair_masked = tf.split(x, [1, 1, 1], 4)
    ...
    # T2 layers
    # Layer #1
    t2_conv1 = conv_3d(t2, 32, 3, activation='tanh', weights_init='xavier', padding='same', regularizer="L2")
    t2_conv1 = conv_3d(t2_conv1, 32, 3, activation='tanh', weights_init='xavier', padding='same', regularizer="L2")
    t2_pool1 = max_pool_3d(t2_conv1, 2)
    t2_concat = tf.concat([fl_pool1, t2_pool1], 4)
    # Layer #2
    t2_conv2 = conv_3d(t2_concat, 64, 3, activation='tanh', weights_init='xavier', padding='same', regularizer="L2")
    t2_conv2 = conv_3d(t2_conv2, 64, 3, activation='tanh', weights_init='xavier', padding='same', regularizer="L2")
    t2_pool2 = max_pool_3d(t2_conv2, 2)
    # Layer #3
    t2_conv3 = conv_3d(t2_pool2, 128, 3, activation='tanh', weights_init='xavier', padding='same', regularizer="L2")
    t2_conv3 = conv_3d(t2_conv3, 128, 3, activation='tanh', weights_init='xavier', padding='same', regularizer="L2")
    t2_pool3 = max_pool_3d(t2_conv3, 2)
    # Dense layer
    t2_pred_conv = conv_3d(t2_pool3, 1, 1, activation='linear', padding='valid')
    ...
    concat = tf.concat([pred_conv, t2_pred_conv, fl_pred_conv], 4)
